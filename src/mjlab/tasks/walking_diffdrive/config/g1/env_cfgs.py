"""Unitree G1 upright differential-drive WALKING: tank-style twist command over the upright
walk gait library.
...
``crawling_common.library.LIBRARY_SPECS["walking_diffdrive"]``; build/refresh it with
``uv run python -m mjlab.scripts.build_library walking_diffdrive``.
"""
...
SPEC = LIBRARY_SPECS["walking_diffdrive"]
...
def unitree_g1_walking_diffdrive_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
