...
class LibraryMotionCommand(MotionCommand):
  ...
  def __init__(self, cfg: LibraryMotionCommandCfg, env: ManagerBasedRlEnv):
    ...
    # Envs whose sampled speed is below the stop threshold enter a "belly rest": the phase is
    # frozen (below), imitation is gated off, and rest rewards take over (see rewards/env_cfgs).
    self.is_stopped = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
  ...
  def _resample_speed(self, env_ids: torch.Tensor) -> None:
    ...
    Speeds below ``stop_speed_threshold`` flag the env as stopped (belly rest) and pin its
    commanded speed to exactly 0 so the forward-speed reward wants a full halt during rest."""
    lo, hi = self.cfg.speed_command_range
    v = sample_uniform(lo, hi, (len(env_ids),), device=self.device)
    stopped = v < self.cfg.stop_speed_threshold
    self.is_stopped[env_ids] = stopped
    self.speed_command[env_ids] = torch.where(stopped, torch.zeros_like(v), v)
  ...
  def _update_command(self) -> None:
    # Freeze the phase for stopped envs (hold current frame); advance moving envs by one step.
    # Mirrors MotionCommand._update_command with the single-step increment gated by is_stopped.
    self.time_steps += (~self.is_stopped).long()
...
class LibraryMotionCommandCfg(MotionCommandCfg):
  ...
  motion_dir: str = ""
  speed_command_range: tuple[float, float] = (0.0, 0.40)
  # Commanded speeds below this enter belly-rest (phase frozen, imitation off, rest rewards on).
  stop_speed_threshold: float = 0.08
