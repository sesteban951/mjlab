...
class TvlqrGuidedJointPositionAction(JointPositionAction):
  ...
  def __init__(self, cfg: TvlqrGuidedJointPositionActionCfg, env: ManagerBasedRlEnv):
    ...
    if abs(self._dt_export - env.physics_dt) > 1e-12:
      raise ValueError(
        f"the export is at dt={self._dt_export} but physics_dt={env.physics_dt}; K was designed "
        f"for the export rate and applying it at another is a different controller"
      )
  ...
  def process_actions(self, actions: torch.Tensor) -> None:
    ...
    self._k0 = torch.clamp(cmd.time_steps * self._decimation, max=self._n_steps - 1)
    self._substep = 0
    self._violation.zero_()
    self._violation_rel.zero_()

  def apply_actions(self) -> None:
    k = torch.clamp(self._k0 + self._substep, max=self._n_steps - 1)
    self._substep += 1
    ...
    if self.cfg.residual:
      ...
      self._entity.set_joint_position_target(
        qdes + self._processed_actions - self._offset - encoder_bias,
